	...
	float* dL_dcov)
{
	...
	double dL_dN = (P0)*dL_dconic.x + (0.5f*P2)*dL_dconic.y + (P1)*dL_dconic.z;
	...
	double dL_dP0 = (N)*dL_dconic.x + (-9.0f*point_xy.x*point_xy.x/(N_dev*N_dev))*dL_dN + (-4.0f*P1*point_xy.x/xy_param.z)*dL_dxc + ((2.0f*P4 - 4.0f*P1*point_xy.y)/xy_param.z)*dL_dyc;
	double dL_dP1 = (N)*dL_dconic.z + (-9.0f*point_xy.y*point_xy.y/(N_dev*N_dev))*dL_dN + ((2.0f*P3 - 4.0f*P0*point_xy.x)/xy_param.z)*dL_dxc + (-4.0f*P0*point_xy.y/xy_param.z)*dL_dyc;
	double dL_dP2 = (0.5f*N)*dL_dconic.y + (-9.0f*point_xy.x*point_xy.y/(N_dev*N_dev))*dL_dN + ((-P4 + 2.0f*P2*point_xy.x)/xy_param.z)*dL_dxc + ((-P3 + 2.0f*P2*point_xy.y)/xy_param.z)*dL_dyc;
	...
	float* __restrict__ dL_dcolors)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float dL_dmean2D_norm = dL_dG * dG_ddelx * d.x + dL_dG * dG_ddely * d.y;
			atomicAdd(&dL_dmean2D[global_id].z, dL_dmean2D_norm);
